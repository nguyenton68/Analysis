...
void temp::Loop()
{
  ...
  TFile *fnew = new TFile(Form("%d_n2_xs.root",runnum),"RECREATE");
  ...
  double Mhe3=2.808;// he3 mass GeV
  ...
  else if(runnum==2082||runnum==2085)
    {
      w_min =-0.0093; w_max = 0.0087;
    }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    // double w_rec = sqrt(Mn2**2+2*Mn2*(Ebeam-E_pr)-4*Ebeam*E_pr*sin(ang/2.)*sin(ang/2.))-Mn2;
      ...
      if((ph_foc<=acc_edge[0]||ph_foc<=acc_edge[5])&&ph_foc>=acc_edge[2]&&ph_foc<=acc_edge[4]&&y_foc<=acc_edge[3]&&y_foc>=acc_edge[1])
	{
            ...
            //w_rec = sqrt(Mn2**2+2*Mn2*(Ebeam-E_pr)-4*Ebeam*E_pr*sin(ang/2.)*sin(ang/2.))-Mn2;
