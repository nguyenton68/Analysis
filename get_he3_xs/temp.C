...
void temp::Loop()
{
  ...
  TFile *fnew = new TFile(Form("%d_xs_155data_51pars.root",runnum),"RECREATE");
  TTree * tree = new TTree("t1","new tree"); 
  Float_t xfocal,yfocal,phfocal,thfocal,gold_dp,rec_phi,rec_theta,rec_w2,rec_y,helicity;
  ...
  double totsh_e, deltat, y_rec, th_rec, ph_rec, hel, momentum, E_pr, ang, w_rec;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if((R_tr_n==1)&&(R_cer_asum_c>350)&&(R_ps_e/momentum>0.105)&&(totsh_e/momentum>0.78)&&((evtype&(1<<1))>0)
       ){
      ...
      if((ph_foc<=acc_edge[0]||ph_foc<=acc_edge[5])&&ph_foc>=acc_edge[2]&&ph_foc<=acc_edge[4]&&y_foc<=acc_edge[3]&&y_foc>=acc_edge[1])
	{
	    ...
	    w_rec = sqrt(Mhe3**2+2*Mhe3*(Ebeam-E_pr)-4*Ebeam*E_pr*sin(ang/2.)*sin(ang/2.))-Mhe3;

	  /* Analysis cut: angles, ytg, w*/
	    ana_ph[0] = 0.012683;//0.010548;//0.013065;
	    ana_ph[1] = 0.009747;//0.008379;//0.008647;
	    ana_ph[2] = -0.004306;//-0.005884;//-0.005693;
	    ana_ph[3] = 0.001018;//-0.000895;//0.0;

	    ana_th[0] = 0.045982;//0.046104;//0.044567;
	    ana_th[1] = 0.031564;//0.031778;//0.032660;
	    ana_th[2] = 0.035552;//0.035624;// 0.037486;
	    ana_th[3] = 0.047413;//0.047274;//0.047139;
	  ...
	  if(th_rec>=ana_edge[0]&&th_rec>=ana_edge[1]&&th_rec<=ana_edge[2]&&th_rec<=ana_edge[3]
